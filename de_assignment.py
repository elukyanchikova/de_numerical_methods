...
'''method for computing the y value at x_c (current) corresponding to IVP (according to the Exact solution)
    y(x_c) = 1 / (const * exp(-3*x*x/2) - (1/3))  '''


def exact_solution(x_c, const):
    y = 1 / (const * np.exp((-3 * x_c * x_c) / 2) - (1 / 3))
    return y
...
x0 = float(input("Input x0: "))  # 1
y0 = float(input("Input y0: "))  # 3
xf = float(input("Input max x: "))  # 5.5
